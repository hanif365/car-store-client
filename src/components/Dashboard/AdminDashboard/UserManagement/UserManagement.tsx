...
const UserManagement = () => {
  ...
  const handleToggleStatus = async (user: User) => {
    try {
      ...
      toast.success(`User ${user.name} ${user.isActive ? 'deactivated' : 'activated'} successfully`);
    } catch (error) {
      toast.error(
  ...
  if (isError) {
    return <div className="flex justify-center items-center h-full text-red-500">Error loading users</div>;
  }
